...
function toJS() {
    return schematics_1.chain([
        schematics_1.forEach(schematics_1.when((path) => path.endsWith('.ts') || path.endsWith('.tsx'), (entry) => {
...
function updateTsConfigsToJs(options) {
    const paths = {
        tsConfig: core_1.normalize(`${options.projectRoot}/tsconfig.json`),
        tsConfigLib: core_1.normalize(`${options.projectRoot}/tsconfig.lib.json`),
        tsConfigApp: core_1.normalize(`${options.projectRoot}/tsconfig.app.json`),
    };
    ...
    return schematics_1.chain([
        ast_utils_1.updateJsonInTree(paths.tsConfig, (json) => {
        ...
        (tree) => {
            ...
            return ast_utils_1.updateJsonInTree(updateConfigPath, (json) => {
