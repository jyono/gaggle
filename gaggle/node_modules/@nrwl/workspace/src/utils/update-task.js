...
function addUpdateTask(pkg, to, dependencies = []) {
    return (host, context) => {
        ...
        const packageJson = ast_utils_1.readJsonInTree(host, 'package.json');
    ...
    return {
        ...
        create: () => {
            return Promise.resolve((options, context) => {
                ...
                const ng = path_1.join('./node_modules', '@angular/cli', 'bin/ng');
                const args = [
                    'update',
                ...
                return new rxjs_1.Observable((obs) => {
                    child_process_1.fork(ng, args, forkOptions).on('close', (code) => {
                        if (code === 0) {
                            obs.next();
                            obs.complete();
                        }
