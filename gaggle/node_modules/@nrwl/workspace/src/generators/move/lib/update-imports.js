...
function updateImports(tree, schema, project) {
    ...
    const projectRef = {
        from: fromPath ||
            utils_1.normalizeSlashes(`@${npmScope}/${project.root.substr(libsDir.length + 1)}`),
        to: schema.importPath ||
            utils_1.normalizeSlashes(`@${npmScope}/${schema.destination}`),
    };
    ...
    if (tsConfig) {
        ...
        devkit_1.writeJson(tree, tsConfigPath, tsConfig);
    }
}
...
function updateImportDeclarations(sourceFile, from, to) {
    const importDecls = find_nodes_1.findNodes(sourceFile, ts.SyntaxKind.ImportDeclaration);
    const changes = [];
...
function updateDynamicImports(sourceFile, from, to) {
    const expressions = find_nodes_1.findNodes(sourceFile, ts.SyntaxKind.CallExpression);
    const changes = [];
