...
function removeProjectConfig(tree, schema) {
    devkit_2.removeProjectConfiguration(tree, schema.projectName);
    // Unset default project if deleting the default project
    const workspaceConfiguration = devkit_2.readWorkspaceConfiguration(tree);
    if (workspaceConfiguration.defaultProject &&
        workspaceConfiguration.defaultProject === schema.projectName) {
        const workspacePath = devkit_2.getWorkspacePath(tree);
    ...
    devkit_1.getProjects(tree).forEach((project, projectName) => {
        ...
        devkit_1.updateProjectConfiguration(tree, projectName, project);
    });
}
