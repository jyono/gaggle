...
function addPropertyToJestConfig(host, path, propertyName, value) {
    ...
    try {
        const configObject = functions_1.jestConfigObjectAst(host.read(path).toString('utf-8'));
        const properties = propertyName.split('.');
        functions_1.addOrUpdateProperty(host, configObject, properties, JSON.stringify(value), path);
    }
    catch (e) {
...
function removePropertyFromJestConfig(host, path, propertyName) {
    ...
    try {
        ...
        if (propertyAssignment) {
            ...
            const updatedFile = devkit_1.applyChangesToString(file, [
                {
