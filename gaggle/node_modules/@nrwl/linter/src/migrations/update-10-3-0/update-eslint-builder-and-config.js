    ...
    return workspace_1.updateBuilderConfig((options, target, project) => {
        ...
        if (options.tsConfig) {
            ...
            normalizedTsConfigOption.forEach((tsConfigPath) => {
                try {
                    tsconfigs.push(workspace_1.readJsonInTree(host, tsConfigPath));
                }
                catch (_a) { }
                try {
                    tsconfigs.push(workspace_1.readJsonInTree(host, `${project.root}/tsconfig.json`));
                }
                catch (_b) { }
            });
        }
            ...
            try {
                tsconfigs.push(workspace_1.readJsonInTree(host, `${project.root}/tsconfig.app.json`));
            }
            catch (_b) { }
            try {
                tsconfigs.push(workspace_1.readJsonInTree(host, `${project.root}/tsconfig.lib.json`));
            }
            catch (_c) { }
            try {
                tsconfigs.push(workspace_1.readJsonInTree(host, `${project.root}/tsconfig.spec.json`));
            }
            catch (_d) { }
            try {
                tsconfigs.push(workspace_1.readJsonInTree(host, `${project.root}/tsconfig.e2e.json`));
            }
            catch (_e) { }
        }
        ...
        let lintFilePatterns = !tsconfigs.length
            ...
            : tsconfigs
                ...
                .map((pattern) => core_1.join(core_1.normalize(project.root), pattern))
                /**
...
function default_1() {
    return schematics_1.chain([updateESLintBuilder, workspace_1.formatFiles()]);
}
